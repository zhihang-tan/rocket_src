...
object NBDcacheReadyValidIO {

  implicit class AddMethodsToReadyValid[T <: Data](target: NBDcacheReadyValidIO[T]) {
...
/** A concrete subclass of NBDcacheReadyValidIO signaling that the user expects a
  ...
  * of ready or valid.
  * @param gen the type of data to be wrapped in NBDcacheDecoupledIO
  */
class NBDcacheDecoupledIO[+T <: Data](gen: T) extends NBDcacheReadyValidIO[T](gen) {

  /** Applies the supplied functor to the bits of this interface, returning a new
    * typed NBDcacheDecoupledIO interface.
    * @param f The function to apply to this NBDcacheDecoupledIO's 'bits' with return type B
    * @return a new NBDcacheDecoupledIO of type B
    */
  def map[B <: Data](f: T => B): NBDcacheDecoupledIO[B] = {
    val _map_bits = f(bits)
    val _map = Wire(NBDcacheDecoupled(chiselTypeOf(_map_bits)))
    _map.bits := _map_bits
    _map.valid := valid
    ready := _map.ready
    _map
...
object NBDcacheDecoupled {

  /** Wraps some Data with a NBDcacheDecoupledIO interface. */
  def apply[T <: Data](gen: T): NBDcacheDecoupledIO[T] = new NBDcacheDecoupledIO(gen)

  // TODO: use a proper empty data type, this is a quick and dirty solution
  private final class EmptyBundle extends Bundle

  // Both of these methods return NBDcacheDecoupledIO parameterized by the most generic type: Data
  /** Returns a [[NBDcacheDecoupledIO]] inteface with no payload */
  def apply(): NBDcacheDecoupledIO[Data] = apply(new EmptyBundle)

  /** Returns a [[NBDcacheDecoupledIO]] inteface with no payload */
  def empty: NBDcacheDecoupledIO[Data] = NBDcacheDecoupled()

  /** Downconverts an IrrevocableIO output to a NBDcacheDecoupledIO, dropping guarantees of irrevocability.
    *
    * @note unsafe (and will error) on the producer (input) side of an IrrevocableIO
    */
  def apply[T <: Data](irr: IrrevocableIO[T]): NBDcacheDecoupledIO[T] = {
    require(
      DataMirror.directionOf(irr.bits) == Direction.Output,
      "Only safe to cast produced Irrevocable bits to Decoupled."
    )
    val d = Wire(new NBDcacheDecoupledIO(chiselTypeOf(irr.bits)))
    d.bits := irr.bits
    d.valid := irr.valid
    irr.ready := d.ready
    d
...
/** A concrete subclass of NBDcacheReadyValidIO that promises to not change
  ...
  * @groupdesc Signals The actual hardware fields of the Bundle
  */
class IrrevocableIO[+T <: Data](gen: T) extends NBDcacheReadyValidIO[T](gen)
...
object Irrevocable {
  ...
  /** Upconverts a NBDcacheDecoupledIO input to an IrrevocableIO, allowing an IrrevocableIO to be used
    * where a NBDcacheDecoupledIO is expected.
    *
    * @note unsafe (and will error) on the consumer (output) side of an NBDcacheDecoupledIO
    */
  def apply[T <: Data](dec: NBDcacheDecoupledIO[T]): IrrevocableIO[T] = {
    require(
...
/** Producer - drives (outputs) valid and bits, inputs ready.
  ...
  */
object EnqIO {
  def apply[T <: Data](gen: T): NBDcacheDecoupledIO[T] = NBDcacheDecoupled(gen)
}
...
/** Consumer - drives (outputs) ready, inputs valid and bits.
  ...
  */
object DeqIO {
  def apply[T <: Data](gen: T): NBDcacheDecoupledIO[T] = Flipped(NBDcacheDecoupled(gen))
}
...
class NBDcacheQueueIO[T <: Data](
   ...
   *  of the interface so uses the flipped in-stance.
   */
  /** I/O to enqueue data (client is producer, and Queue object is consumer), is [[Chisel.NBDcacheDecoupledIO]] flipped.
    * @group Signals
    */
  val enq = Flipped(EnqIO(gen))

  /** I/O to dequeue data (client is consumer and Queue object is producer), is [[Chisel.NBDcacheDecoupledIO]]
    * @group Signals
    */
...
class NBDcacheQueue[T <: Data](
  ...
  val io = IO(new NBDcacheQueueIO(genType, entries, hasFlush))
  ...
  /*runahead code begin*/
  io.deq_ptr_value := deq_ptr.value
  io.enq_ptr_value := enq_ptr.value
  /*runahead code end*/
...
object NBDcacheQueue {

  /** Create a [[Queue]] and supply a [[NBDcacheDecoupledIO]] containing the product.
    *
  ...
  def apply[T <: Data](
    enq:            NBDcacheReadyValidIO[T],
    entries:        Int = 2,
    pipe:           Boolean = false,
    flow:           Boolean = false,
    useSyncReadMem: Boolean = false,
    flush:          Option[Bool] = None
  ): NBDcacheDecoupledIO[T] = {
    if (entries == 0) {
      val deq = Wire(new NBDcacheDecoupledIO(chiselTypeOf(enq.bits)))
      deq.valid := enq.valid
      deq.bits := enq.bits
      enq.ready := deq.ready
      deq
    } else {
      val q = Module(new NBDcacheQueue(chiselTypeOf(enq.bits), entries, pipe, flow, useSyncReadMem, flush.isDefined))
  ...
  /** Create a queue and supply a [[IrrevocableIO]] containing the product.
    * Casting from [[NBDcacheDecoupledIO]] is safe here because we know the [[Queue]] has
    * Irrevocable semantics.
    * we didn't want to change the return type of apply() for backwards compatibility reasons.
    *
    * @param enq [[NBDcacheDecoupledIO]] signal to enqueue.
    * @param entries The max number of entries in the queue
    * @param pipe True if a single entry queue can run at full throughput (like a pipeline). The ''ready'' signals are
    * combinationally coupled.
    * @param flow True if the inputs can be consumed on the same cycle (the inputs "flow" through the queue immediately).
    ...
    * @param flush Optional [[Bool]] signal, if defined, the [[Queue.hasFlush]] will be true, and connect correspond
    *              signal to [[Queue]] instance.
    * @return a [[NBDcacheDecoupledIO]] signal which should connect to the dequeue signal.
    *
    * @example {{{
    *   consumer.io.in <> Queue(producer.io.out, 16)
    * }}}
